...
import { PLAYER_SIZE, PLAYER_BIG_WIDTH, PLAYER_BIG_HEIGHT, PLAYER_JUMP_VELOCITY, STACHE_BALL_SPEED, PLAYER_DEPTH, MAX_JUMPS, PLAYER_FIRE_COOLDOWN, PLAYER_BALL_OFFSET_LEFT, PLAYER_BALL_OFFSET_RIGHT, PLAYER_INVULNERABILITY_DURATION, PLAYER_HIT_FLASH_ALPHA, PLAYER_HIT_FLASH_DURATION, PLAYER_HIT_FLASH_REPEAT, PLAYER_DEAD_DEPTH, PLAYER_DEATH_PAUSE, PLAYER_DEATH_RISE, PLAYER_DEATH_RISE_DURATION, PLAYER_DEATH_FALL_TARGET, PLAYER_DEATH_FALL_DURATION, PLAYER_SIZE_CHANGE_DURATION, PLAYER_POWERUP_FLASH_ALPHA, PLAYER_POWERUP_FLASH_DURATION, PLAYER_POWERUP_FLASH_REPEAT, PLAYER_WIN_WALK_DURATION, PLAYER_WIN_DELAY, PLAYER_UNCROUCH_Y_DIVISOR, WARP_PIPE_ENTRY_OVERLAP, WARP_PIPE_ENTRY_DURATION, } from '../../constants';
...
export class Mustachio extends Phaser.Physics.Arcade.Sprite {
    ...
    constructor(scene, x, y, isBig, isFire) {
    ...
    fire() {
        if (!this.isFire || !this.canFireProjectile)
            return;
        this.canFireProjectile = false;
        this.scene.time.delayedCall(PLAYER_FIRE_COOLDOWN, () => {
            this.canFireProjectile = true;
        });
        const dirX = this.facingDirection === 'left' ? -STACHE_BALL_SPEED : STACHE_BALL_SPEED;
        const ballX = this.facingDirection === 'left'
    ...
    playerHit() {
        ...
        if (this.hitTimer !== null || this.scene.gameOver)
            return;
        if (this.isFire) {
        ...
        this.hitTimer = this.scene.time.delayedCall(PLAYER_INVULNERABILITY_DURATION, () => {
            this.hitTimer = null;
        });
        // Flash effect
    ...
    changeSize(big, showAnimation = true) {
        if (this.isBig === big)
            return;
        this.isBig = big;
    ...
    changeFire(fire, showAnimation = true) {
        if (this.isFire === fire)
            return;
        this.isFire = fire;
        if (fire && !this.isBig) {
