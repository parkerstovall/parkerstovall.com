import { STACHE_SHOT_SPEED, GAME_WIDTH, GAME_HEIGHT, ENEMY_SIZE, STACHE_SHOT_CULL_MARGIN, } from '../../constants';
...
export class StacheShot extends Enemy {
    pointValue = 250;
    constructor(scene, parentX, parentY, parentWidth, parentHeight, dir) {
        const size = ENEMY_SIZE;
        const x = parentX + parentWidth / 2 - size / 2;
        const y = parentY + parentHeight / 2 - size / 2;
        let texKey;
        if (dir === Direction.LEFT)
            texKey = 'stache-shot-left';
        else if (dir === Direction.RIGHT)
            texKey = 'stache-shot-right';
        else if (dir === Direction.UP)
            texKey = 'stache-shot-up';
        else
            texKey = 'stache-shot-down';
        ...
        if (dir === Direction.LEFT)
            this.setVelocityX(-STACHE_SHOT_SPEED);
        else if (dir === Direction.RIGHT)
            this.setVelocityX(STACHE_SHOT_SPEED);
        else if (dir === Direction.UP)
            this.setVelocityY(-STACHE_SHOT_SPEED);
    ...
    preUpdate(time, delta) {
        ...
        if (this.x < cam.scrollX - STACHE_SHOT_CULL_MARGIN ||
