import { STREAKER_SPEED, STREAKER_WIDTH, STREAKER_HEIGHT, STREAKER_PATROL_DISTANCE, STREAKER_SHOT_COOLDOWN, STREAKER_SHOT_DURATION, } from '../../constants';
...
export class StacheStreaker extends Enemy {
    ...
    fireLaser() {
        if (this.isDead || !this.active)
            return;
        new Laser(this.scene, this.x, this.y, this.displayWidth, this.displayHeight, STREAKER_SHOT_DURATION);
        this.canMove = false;
        this.setVelocityX(0);
        this.nextImage();
        this.shotTimer = this.scene.time.delayedCall(STREAKER_SHOT_DURATION, () => {
            ...
            this.setVelocityX(STREAKER_SPEED);
            this.nextImage();
            this.shotTimer = this.scene.time.delayedCall(STREAKER_SHOT_COOLDOWN, () => {
                this.fireLaser();
            });
        });
    }
