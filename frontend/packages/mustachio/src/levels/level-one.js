...
import { createBlockPyramid, createBlockSquare, createBlockWall, } from './level-helpers';
...
export function levelOne(scene, previousLevels = []) {
    sectionOne(scene);
    sectionTwo(scene);
...
function sectionThree(scene, previousLevels) {
...
function sectionSix(scene) {
    for (let i = 0; i < 5; i++) {
        new StacheCannon(scene, BLOCK_SIZE * 143, BLOCK_SIZE * (i * 3 + 2), Direction.RIGHT);
    }
